...
def judge():
    ...
    if num1.isdigit() and num2.isdigit() and num1<=num2:
        ...
        showinfo('showinfo','ValueError:empty range for randrange()')
    else:
        showinfo('showinfo','SyntaxError:invalid character')


root=Tk()
root.title('Print random integer')
...
Button(root,text='Print',font=('NSimSun',15),command=judge).place(relx=0.4,rely=0.6,relheight=0.1,relwidth=0.2)
